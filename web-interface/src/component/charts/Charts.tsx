    ...
    return <div className="charts">
        <AppBarOneRow>
            ...
            <Typography variant="h6">
                {props.title(measurementType as string)}
            </Typography>
        </AppBarOneRow>
        <main>
            ...
            {props.airQualityDataAverages.loadingState == LoadingState.success && <Paper elevation={2} className="charts-container">
                <Chart title="Daily" unitMeter={props.unitMeter(measurementType as string)} averages={props.airQualityDataAverages.dailyAverages.map(da => ({
                    ...da,
                    average: da.value / da.counter,
                    xaxis: da.timeRange.substring(0, 4) + '-' + da.timeRange.substring(4, 6) + '-' + da.timeRange.substring(6)
                }))} />
                <Chart title="Montly" unitMeter={props.unitMeter(measurementType as string)} averages={props.airQualityDataAverages.monthlyAverages.map(da => ({
                    ...da,
                    average: da.value / da.counter,
                    xaxis: da.timeRange.substring(0, 4) + '-' + da.timeRange.substring(4)
                }))} />
                <Chart title="Yearly" unitMeter={props.unitMeter(measurementType as string)} averages={props.airQualityDataAverages.yearlyAverages.map(da => ({
                    ...da,
                    average: da.value / da.counter,
                    xaxis: da.timeRange
                }))} />
